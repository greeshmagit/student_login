...
pipeline{
   agent any
      stages{
       	stage("docker build"){
      	    steps{
              		 sh "docker build -t pgreeshma/login-html:v2 ."
          	 }
       }
      ...
      steps{
         sshagent(['tomcat']){
                    ...
                    sh "ssh -o StrictHostkeyChecking=no ec2-user@172.31.34.138 docker run -d -p 80:80 --name nginx pgreeshma/login-html:v2"
                  }
             }
        }  
    }
